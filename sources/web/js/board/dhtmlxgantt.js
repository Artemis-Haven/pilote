...
function getOffsetSum(t) {
    for (var e = 0, n = 0; t; )
        e += parseInt(t.offsetTop), n += parseInt(t.offsetLeft), t = t.offsetParent;
    return{top: e, left: n};
}
...
window.dhtmlx || (dhtmlx = function(t) {
    ...
    if (this.xmlDoc.responseXML) {
        ...
        if (0 === i.length && -1 != t.indexOf(":"))
            var i = this.xmlDoc.responseXML.getElementsByTagName(t.split(":")[1]);
        n = i[0]
    } else
        n = this.xmlDoc.documentElement;
    if (n)
        return this._retry = !1, n;
    ...
    if (_isIE)
        return e || (e = this.xmlDoc.nodeName ? this.xmlDoc : this.xmlDoc.responseXML), e || dhtmlxError.throwError("LoadXML", "Incorrect XML", [e || this.xmlDoc, this.mainObject]), n && e.setProperty("SelectionNamespaces", "xmlns:xsl='" + n + "'"), "single" == i ? e.selectSingleNode(t) : e.selectNodes(t) || new Array(0);
    var a = e;
    ...
    for (var n = [], e = e.replace(/[^\[]*\[\@/g, "").replace(/[\[\]\@]*/g, ""), i = 0; i < t.length; i++)
        t[i].getAttribute(e) && (n[n.length] = t[i]);
    return n
}, dtmlXMLLoaderObject.prototype._getAllNamedChilds = function(t, e) {
    var n = [];
    _isKHTML && (e = e.toUpperCase());
    for (var i = 0; i < t.length; i++)
        for (var a = 0; a < t[i].childNodes.length; a++)
            _isKHTML ? t[i].childNodes[a].tagName && t[i].childNodes[a].tagName.toUpperCase() == e && (n[n.length] = t[i].childNodes[a]) : t[i].childNodes[a].tagName == e && (n[n.length] = t[i].childNodes[a]);
    return n
    ...
    if (_isIE) {
        ...
        this.XSLProcessor || (this.XSLProcessor = new XSLTProcessor, this.XSLProcessor.importStylesheet(t)), n = this.XSLProcessor.transformToDocument(e);
    return n
    ...
    if (t || (t = this.xmlDoc), t.responseXML && (t = t.responseXML), _isIE)
        return t.xml;
    ...
    if (t && "object" == typeof t) {
        for (i = {}, n = [Array, Date, Number, String, Boolean], e = 0; e < n.length; e++)
    ...
    for (; t; )
        e += parseInt(t.offsetTop, 10), n += parseInt(t.offsetLeft, 10), t = t.offsetParent;
    ...
    for (var a = !0; e && e.parentNode; ) {
        ...
        if (s) {
            ...
            for (var r = 0; r < s.length; r++)
                s[r] && n[s[r]] && (a = n[s[r]].call(gantt, t, i, e), a = !("undefined" != typeof a && a !== !0))
        }
        e = e.parentNode
    }
    return a
    ...
    if (gantt.checkEvent("onMouseMove")) {
        ...
        if (!this.config.marker && !this.config.started) {
            ...
            if (s > this.config.sensitivity) {
                if (this.config.started = !0, this.config.ignore = !1, this.callEvent("onBeforeDragStart", [t, e]) === !1)
...
//********************************
        ...
        if (this.callEvent("onGridHeaderClick", [i, t]))
            if ("add" == i)
                this._click.gantt_add(t, this.config.root_id);
    ...
    for (var e = this.config.columns, n = [], i = 0; i < e.length; i++) {
        ...
        if (o.tree) {
            for (var _ = 0; _ < t.$level; _++)
                l += this.templates.grid_indent(t);
    ...
    dhtmlx.defined(this.config.dnd_sensitivity) && (t.config.sensitivity = this.config.dnd_sensitivity), t.attachEvent("onBeforeDragStart", dhtmlx.bind(function(t, e) {
            ...
            if (_.y + o.offsetHeight / 2 < r) {
                ...
                if (g) {
                    if (g.id == l.id)
                        return;
                    h = g
                } else if (g = this._pull[this._order[c]], g.$level == l.$level)
                    return this.moveTask(l.id, -1, g.parent), void(i.target = "next:" + g.id)
            }
            if (h.$level == l.$level && l.id != h.id)
                this.moveTask(l.id, 0, 0, h.id), i.target = h.id;
            else {
                if (l.id == h.id)
                    return;
        ...
        if (t.ignore_x = {}, gantt.ignore_time || gantt.config.skip_off_time) {
            ...
            for (var i = 0; i < t.trace_x.length; i++)
                n.call(gantt, t.trace_x[i]) || this._ignore_time_config.call(gantt, t.trace_x[i]) ? (t.ignore_x[t.trace_x[i].valueOf()] = !0, t.ignored_colls = !0) : e++
        }
        t.display_count = e
        ...
        for (var r = t.ignore_x || {}, o = 0; o < t.trace_x.length; o++)
            t.width[o] = r[t.trace_x[o].valueOf()] || t.display_count == t.count ? 0 : 1;
        ...
        for (var s = e.trace_x, r = t.trace_x, o = n || 0, d = i || r.length - 1, l = 0, _ = 1; _ < s.length; _++)
            for (var h = o; d >= h; h++)
                +r[h] != +s[_] || (a && a.apply(this, [l, _, o, h]), o = h, l = _)
        ...
        for (var s = new Date(i); + a > + s; )
            n.call(this, new Date(s)), s = gantt.date.add(s, e, t)
        ...
        if (2 != t.button && !gantt.config.readonly && !this.drag.mode) {
            ...
            if (i)
                if (i.mode && i.mode != gantt.config.drag_mode.ignore && gantt.config["drag_" + i.mode]) {
                    ...
                    if (gantt._is_flex_task(s) && i.mode != gantt.config.drag_mode.progress)
                    ...
                    this.clear_drag_state();
            else if (gantt.checkEvent("onMouseDown") && gantt.callEvent("onMouseDown", [n.split(" ")[0]]) && e.parentNode)
        ...
        if (e.mode && e.id) {
            ...
            if (gantt.config.work_time && gantt.config.correct_work_time && this._fix_working_times(n, e), this._fix_dnd_scale_time(n, e), gantt._init_task_timing(n), this._fireEvent("before_finish", e.mode, [e.id, e.mode, dhtmlx.copy(e.obj), t])) {
                ...
                e.obj._dhx_changed = !1, dhtmlx.mixin(n, e.obj, !0), gantt.updateTask(n.id)
        }
        this.clear_drag_state()
    ...
    var a = null;
    if (a = "object" == typeof t ? t : {source: t, target: e, type: this._get_link_type(n, i)}, !a)
        return!1;
    ...
    var e = this._path_builder.get_points(t), n = gantt._drawer, i = n.get_lines(e), a = document.createElement("div"), s = "gantt_task_link", r = this.templates.link_class ? this.templates.link_class(t) : "";
    r && (s += " " + r), a.className = s, a.setAttribute(gantt.config.link_attribute, t.id);
    for (var o = 0; o < i.length; o++)
        o == i.length - 1 && (i[o].size -= gantt.config.link_arrow_size), a.appendChild(n.render_line(i[o], i[o + 1]));
    ...
    return t * gantt.config.row_height
        ...
        if (t.type == gantt.config.links.start_to_start)
            ...
            if (o = e.e_x > e.x + 2 * r, this.point_to(s.right, r), o)
                a -= r, this.point_to(s.down, i), this.point_to(s.right, a);
            else {
                a -= 2 * r;
                var d = i > 0 ? 1 : -1;
            ...
            this.point_to(s.right, r), o ? (this.point_to(s.right, a), this.point_to(s.down, i)) : (this.point_to(s.down, i), this.point_to(s.right, a)), this.point_to(s.left, r);
    ...
    function t(t, e, n, i) {
        for (var a = 0; a < t.length; a++)
            t[a].change_id(e, n), t[a].render_item(i)
    }
    ...
    if (t.bar_height = this._get_task_height(), this.$task_data.style.height = Math.max(this.$task.offsetHeight - this.config.scale_height, 0) + "px", this.$task_bg.style.width = t.full_width + "px", this._is_grid_visible()) {
        for (var e = this.config.columns, n = 0, i = 0; i < e.length; i++)
            n += e[i].width;
    ...
    if (this.config.start_date && this.config.end_date)
        return this._min_date = this.date[t + "_start"](new Date(this.config.start_date)), void(this._max_date = this.date[t + "_start"](new Date(this.config.end_date)));
    ...
    if (i && this.isTaskVisible(t))
        ...
        for (var a = 0; a < n.length; a++)
            n[a].remove_item(t);
    if (void 0 === e || e) {
        for (var i = this.getTask(t), a = 0; a < i.$source.length; a++)
    ...
    var e = this._tasks;
    if (0 > t || t > e.full_width)
        return null;
    for (var n = 0, i = 0; i + e.width[n] < t; )
        i += e.width[n], n++;
    ...
    for (var n = Math.floor(e), i = e % 1, a = 0, s = 1; n >= s; s++)
        a += gantt._tasks.width[s - 1];
    ...
    return this._task_area_pulls || (this._task_area_pulls = {}), this._task_area_renderers || (this._task_area_renderers = {}), this._task_area_renderers[t] ? this._task_area_renderers[t] : (e || dhtmlx.assert(!1, "Invalid renderer call"), this._task_area_renderers[t] = {render_item: function(a, s) {
            ...
            for (var a = document.createDocumentFragment(), s = 0, r = e.length; r > s; s++)
        ...
        }, rendered: this._task_area_pulls[t], node: n}, this._task_area_renderers[t])
    ...
    for (var n in this._pull)
        this._pull[n].$level = this._item_level(this._pull[n]);
    ...
    if (i)
        ...
        e = this._getXML(t, e);
        for (var n = {}, i = n.data = [], a = e.doXPath("//task"), s = 0; s < a.length; s++)
    ...
    return e ? this.serverList[t] = e.slice(0) : this.serverList[t] || (this.serverList[t] = []), this.serverList[t]
        ...
        for (var a = new Date(t), s = new Date(e), i = i || 1, r = 0; a.valueOf() < s.valueOf(); )
            this.is_working_unit(a, n) && r++, a = gantt.date.add(a, i, n);
        return r
        ...
        for ("past" == t.dir && (l = - 1); !this.is_working_unit(n, e); )
            if (d && (n = s ? i : a, l = -1 * l), n = gantt.date.add(n, l, e), d && (s ? i = n : a = n), s = !s, o++, o > r)
                return dhtmlx.assert(!1, "Invalid working time check"), !1;
    ...
    if (!this._pull[t])
        return!1;
    if (!(+this._pull[t].start_date < +this._max_date && +this._pull[t].end_date > +this._min_date))
        return!1;
    ...
    for (var a = 0; a < i.length; a++)
        this._silentStart(), this._deleteTask(i[a], !0), this._dp && (this._dp._ganttMode = "tasks", this._dp.setUpdated(i[a], !0, "deleted")), this._silentEnd();
    for (!e && this._dp && this._dp.setUpdateMode("cell"); n.$source.length > 0; )
        this.deleteLink(n.$source[0]);
    for (; n.$target.length > 0; )
        this.deleteLink(n.$target[0]);
    ...
    for (var i in this._pull)
        this._pull[i].parent == t && (this._pull[i].parent = e);
    ...
    if (t) {
        for (var n = this.getTask(t); !n.$no_end && !n.$no_start && n.parent && this.isTaskExists(n.parent); )
            n = this.getTask(n.parent);
        if (n.$no_end) {
            var i = 0;
            this.eachTask(function(t) {
                t.end_date && +t.end_date > +i && (i = new Date(t.end_date))
    ...
    for (var n = this.getTask(t); n && this.isTaskExists(n.parent); )
        if (n = this.getTask(n.parent), n && n.id == e)
            return!0;
    return!1
}, gantt._get_closest_date = function(t) {
    for (var e = t.date, n = t.step, i = t.unit, a = gantt.date[i + "_start"](new Date(this._min_date)); + e > + a; )
        a = gantt.date.add(a, n, i);
    ...
    for (var e = 0; t.parent && dhtmlx.defined(this._pull[t.parent]); )
        t = this._pull[t.parent], e++;
    return e
    ...
    if (s) {
        ...
        for (var o = 0; o < r.length; o++)
            s[o] = r[o].id, this.sort(t, e, s[o], !0)
    }
    i && this.refreshData()
    ...
    for (var t in this._pull)
        this._pull[t].$source = [], this._pull[t].$target = [];
...
//	getTaskDetails($( this ).attr('id').replace('task-', ''));
  ...
  /* 
    ...
    if (void 0 === t && (t = this.getLightboxType()), !this._lightbox || this.getLightboxType() != this._get_safe_type(t)) {
        ...
        for (var s in a)
            i += "<div class='dhx_btn_set dhx_left_btn_set " + a[s] + "_set'><div dhx_button='1' class='" + a[s] + "'></div><div>" + this.locale.labels[a[s]] + "</div></div>";
        a = this.config.buttons_right;
        for (var s in a)
            i += "<div class='dhx_btn_set dhx_right_btn_set " + a[s] + "_set' style='float:right;'><div dhx_button='1' class='" + a[s] + "'></div><div>" + this.locale.labels[a[s]] + "</div></div>";
    ...
    t && t.focus && (gantt.config.touch || (e && t.select && t.select(), t.focus()))
        ...
        for (var o = "", d = 0; d < n.length; d++) {
            ...
            switch (l) {
                case"%Y":
                    ...
                    for (var h = a.getFullYear() - 5, c = 0; 10 > c; c++)
                        _ += "<option value='" + (h + c) + "'>" + (h + c) + "</option>";
                    break;
                case"%m":
                    t._time_format_order[1] = d, t._time_format_order.size++;
                    for (var c = 0; 12 > c; c++)
                        _ += "<option value='" + c + "'>" + this.locale.date.month_full[c] + "</option>";
                    break;
                case"%d":
                    t._time_format_order[0] = d, t._time_format_order.size++;
                    for (var c = 1; 32 > c; c++)
                        _ += "<option value='" + c + "'>" + c + "</option>";
                    break;
                case"%H:%i":
                    var s = 1440, r = 0;
        ...
        if (t[e + i[0]].value = n.getDate(), t[e + i[1]].value = n.getMonth(), t[e + i[2]].value = n.getFullYear(), dhtmlx.defined(i[3])) {
            ...
            for (var e = (t.height || "23") + "px", n = "<div class='dhx_cal_ltext' style='height:" + e + ";'><select style='width:100%;'>", i = 0; i < t.options.length; i++)
            ...
            for (var a in e)
                i.push({key: e[a], label: n["type_" + a]});
            t.options = i;
            var s = t.onchange;
    ...
    this.callEvent("onLightboxChange", [e, this.getLightboxType()])
        ...
        var n = [], i = [];
        for (var a in t)
            t.hasOwnProperty(a) && (n.push(this.serialize_one(t[a], a + this.post_delim)), i.push(a));
        ...
        var n = [];
        for (var i in t)
            t.hasOwnProperty(i) && n.push(this.escape((e || "") + i) + "=" + this.escape(t[i]));
        return n.join("&")
        ...
        if (s || this.set_invalid(t, n), this._uActions && this._uActions[n] && !this._uActions[n](i))
    ...
    if ("gantt_task_cell" == e.className)
        return null;
